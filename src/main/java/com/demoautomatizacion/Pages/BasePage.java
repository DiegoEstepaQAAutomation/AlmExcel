...
public class BasePage {
	...
	public void submit(By elementLocation, File folderPath, String steps,String Evidencia) throws Exception {
		...
		try {
			...
			} catch (Exception e) {printConsole(e.toString());;} 
	...
	public static String fechaPdf() {
		...
		return fecha.format(fechaSistema);
	}
	...
	public static String horaPdf() {
		...
		return hora.format(horaSistema);
	}
	...
	public static String horaSistema() {
		...
		return horaS.format(horaSistema);
	}
	...
	//Metodos de validacion no usables
	public static boolean validarElementoC(By elementLocation, int time,String Evidencia,String msnError) throws DocumentException {
		try {
			WebDriverWait wait = new WebDriverWait(driver, time);
			wait.until(ExpectedConditions.visibilityOfElementLocated(elementLocation));
			return true ;
		...
		{
			GenerarReportePdf.closeTemplate(Evidencia, msnError);
			return false;
		}
	}
